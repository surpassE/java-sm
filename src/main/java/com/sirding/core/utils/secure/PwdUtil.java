...
public class PwdUtil {
	...
	/**
	 * 默认增强加密方式：采用SHA-512、使用password作为盐值，通过对password计算得到hash次数
	 * @date 2016年10月14日
	 * @author zc.ding
	 * @param password
	 * @return
	 */
	public static ResultType encryptStronger(String password){
		return encrypt(password, DEFAULT_ALGORITHM, password, getIterations(password));
	}
	
	/**
	 * 默认增强加密方式：采用指定的加密方式、使用password作为盐值，通过对password计算得到hash次数
	 * @date 2016年10月17日
	 * @author zc.ding
	 * @param password
	 * @param algorithm
	 * @return
	 */
	public static ResultType encryptStronger(String password, String algorithm){
		return encrypt(password, algorithm, password, getIterations(password));
	}
...
//			return new PwdUtil().new ResultType(hashed);	//不要求ResultType是静态内存类，但每次调用都需要额外创建PwdUtil对象
	 ...
	 * @return
	 */
	private static int getIterations(String password){
		String hex = Hex.encodeHexString(password.getBytes());
		int sum = 1;
		for(int i = 0; i < hex.length(); i++){
			sum += Integer.parseInt(hex.substring(i, i + 1), 16);
		}
		return sum;
	}
